...
| **Histoire clinique** (Sprint 12) | `renderTimelineNarrative`/`fetchTimelineNarrative(id, force)`, `#timelineNarrative`, classes `.tn-*` — carte premium du récit chronologique IA (au-dessus de la chronologie). Actions `copyNarrative` (presse-papiers), `narrativeToCourrier` (pré-remplit l'éditeur de courrier en étape 2), `refreshNarrative` (`?refresh=1`). `narrativePlainText` sérialise les `periodes`. Backend : `GET /api/patients/:id/timeline-narrative` |
