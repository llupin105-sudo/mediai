...
| `SEARCH_INTERPRET_PROMPT` | `POST /api/search/interpret` | **Sprint 12** — Smart Search inter-patients. Traduit une requête clinique en langage naturel en critères `{ termes[], types[], mois, annee }` (synonymes : pathologies proches, paramètres bio, médicaments, examens). **Ne reçoit QUE la requête du médecin** (aucune donnée patient) ; le filtrage se fait ensuite **en local, déterministe**, côté client. Sortie bornée/validée côté serveur. Quota IA partagé. |
| `DOSSIER_CHAT_PROMPT` | `POST /api/patients/:id/chat` | **Sprint 12** — « Discuter avec le dossier » (Copilote). Répond aux questions du médecin **uniquement à partir du dossier fourni** (contexte = synthèse + chronologie compacte). Le message complet (contexte + historique + question) est **anonymisé d'un bloc** avant envoi, ré-identifié au retour. Jamais d'invention (« le dossier ne le précise pas » si absent), non décisionnel, ≤ 6 lignes, réponse « à vérifier ». **Conversation non persistée** (aucune donnée patient stockée — règle HDS). Quota IA partagé + rate-limit. |
